...
description: Use before writing or changing any AssemblyAI integration code in this project (Voice Agent API, realtime STT, pre-recorded transcription, token minting, tool calling). Discovery-first workflow, current endpoints/params, and protocol gotchas — verify against live docs before relying on memorized parameter names.
---
...
**Raw HTTP:**
```bash
curl -F 'audio=@call.wav;type=audio/wav' \
  -F 'config={"prompt":"Support call.","keyterms_prompt":["AssemblyAI"]};type=application/json' \
  -H "Authorization: $ASSEMBLYAI_API_KEY" \
...
| `3005` | Session cancelled (server-side error — retry) |

> **Note — this reference was pasted mid-message and cut off here by a length limit.** Section 14's close-code table and anything after it (the doc's own cross-references imply at least a "Section 15" covering common gotchas, e.g. the `speech_models` array-vs-string mismatch) is missing. Before leaning on error-code handling or the gotchas section for real work, re-paste the rest of the source doc or pull it fresh from `https://www.assemblyai.com/docs/llms-full.txt`.

---

## 15. Project-specific note (Deixis)

This project (`CLAUDE.md` at the repo root) uses the **Voice Agent API** (Section 10) exclusively — not raw realtime STT — because it wants managed turn-taking, barge-in, and tool calling in one socket. Two things `CLAUDE.md` flags as unverified that this skill's Section 10 speaks to directly:

- `CLAUDE.md` §6 guesses the connect endpoint might be `/v1/realtime` vs `/v1/ws`, and says the auth header is plain `Authorization`. Per Section 10 above, the endpoint is `wss://agents.assemblyai.com/v1/ws` and **the Voice Agent API is the one product that requires `Authorization: Bearer YOUR_API_KEY`** (or `?token=` for browser clients) — confirm this against live docs before wiring `api/voice-token.ts`, since it contradicts the no-`Bearer` rule that holds everywhere else in AssemblyAI's API.
- `CLAUDE.md` §6 says `agent_id` is sent as the first frame and is mutually exclusive with inline config. Section 10 above instead shows sending `session.update` with inline `system_prompt`/`tools`/etc. directly, no `agent_id` required. Both may be valid (create-once-reuse vs. configure-inline) — verify which the live docs currently recommend before committing `scripts/publish-agent.ts` to one pattern.

Flag either discrepancy to the user the first time it actually matters (wiring the token endpoint in M0, wiring `session.update` in M1) rather than assuming this skill's snapshot overrides `CLAUDE.md`'s — both are unverified and the live docs win per `CLAUDE.md` §3.
